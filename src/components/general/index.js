...
function General({ nodeList }) {
  ...
  const nodeInfo = useSelector(state => state.nodes.value.find(device => device._id === chooseNode));
  ...
  function handleUpdateInfoNode() {
    ...
    api.patch(`${updateNodeByID_PATH}/${chooseNode}`, { name, typeModal: model, desc })
    .then(res => {
      const { message } = res.data;
      if(message === 'update node successfull!') {
        dispatch(updateNode({ id: chooseNode, value: optionsNode }));
        Toast({ type: 'success', message: 'Node đã được cập nhật thông tin!' });
        setModalNode(false);
      }
    })
    .catch(error => {
      if(error.response.data.message === 'You not permission update this node!') {
        Toast({ type: 'error', message: 'Bạn không có quyền cập nhật node này!' });
      }else {
  ...
  return (
    <>
      {/* modal setting node */}
      {/* modalNode */}
      <MaterialDefaultModal open={ modalNode } handleClose={ () => { setModalNode(false) } }>
        <h5>Thông tin node:  { chooseNode.split('').map((char, index, chars) => index > chars.length / 2 ? char : '*').join('') }</h5>
        <div className="lg:min-w-[500px]">
          <Grid sx={{ marginY: '0.5rem' }} container spacing={2}>
            <Grid xs={7}>
              <TextField onChange={(event) => { setOptionsNode(state => ({ ...state, name: event.target.value })) }} fullWidth size="small" id={`name-node-${chooseNode}`} value={ optionsNode?.name || '' } label="Tên node" variant="outlined" />
            </Grid>
            <Grid xs={5}>
            ...
            <Grid xs={12}>
              <TextField
                ...
                multiline
                rows={4}
                value={ optionsNode?.desc ? (optionsNode.desc === 'no-desc' ? '{ Không có mô tả nào! }' : optionsNode.desc) : '' }
                onChange={(event) => { setOptionsNode(state => ({ ...state, desc: event.target.value })) }}
              />
            </Grid>
          </Grid>
          <Button onClick={ handleUpdateInfoNode } sx={{ marginTop: 0.5, position: 'relative', left: '100%', transform: 'translateX(-100%)' }} variant="contained">Cập nhật thông tin</Button>
        </div>
